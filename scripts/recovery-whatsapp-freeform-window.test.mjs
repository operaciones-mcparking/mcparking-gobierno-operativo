...
test("classifies the WhatsApp freeform window states", () => {
  ...
  assert.equal(classifyWhatsappFreeformWindow("2026-07-30T13:00:00.000Z", now).status, "expiring");
  assert.equal(classifyWhatsappFreeformWindow("2026-07-30T11:59:59.000Z", now).status, "closed");
  assert.equal(classifyWhatsappFreeformWindow(null, now).status, "no_inbound");
  assert.equal(classifyWhatsappFreeformWindow("not-a-date", now).status, "no_inbound");
});
...
test("helper selects the most recent valid inbound across sources", async () => {
  const now = Date.parse("2026-07-31T12:00:00.000Z");
  const supabase = createSupabaseMock({
    carts: [{ id: "cart-1", phone_normalized: "56900000000" }],
    tables: {
      ...
      recovery_whatsapp_message_memory_raw_import: [
        { message_at: "2026-07-31T08:00:00.000Z", message_bound_type: "inbound", wa_id_normalized: "56900000000" },
      ],
      recovery_whatsapp_message_memory_import: [
        { message_at: "2026-07-31T10:00:00.000Z", message_bound_type: "inbound", wa_id_normalized: "56900000000" },
      ],
    },
  });
...
test("null or unknown direction does not count as inbound", async () => {
  const supabase = createSupabaseMock({
    ...
    tables: {
      ...
      recovery_whatsapp_message_memory_raw_import: [
        { message_at: "2026-07-31T10:00:00.000Z", message_bound_type: null, wa_id_normalized: "56900000000" },
      ],
      recovery_whatsapp_message_memory_import: [
        { message_at: "2026-07-31T11:00:00.000Z", message_bound_type: "unknown", wa_id_normalized: "56900000000" },
      ],
    },
  });
  ...
  assert.equal(state.status, "no_inbound");
  assert.equal(state.canSendFreeform, false);
});
...
test("empty source tables return no_inbound with null metadata", async () => {
  const supabase = createSupabaseMock({
    carts: [{ id: "cart-1", phone_normalized: "56900000000" }],
    tables: {
      recovery_whatsapp_live_messages: [],
...
test("helper is server-only and reads only explicit inbound sources", () => {
  ...
  assert.match(helper, /\.eq\("message_bound_type",\s*"inbound"\)/);
...
test("POST send route recalculates the window before local insert, updates, or n8n call", () => {
  ...
  assert.match(sendRoute, /WHATSAPP_FREEFORM_WINDOW_CLOSED/);
  assert.match(sendRoute, /WHATSAPP_FREEFORM_WINDOW_NOT_OPEN/);
  assert.match(sendRoute, /WHATSAPP_FREEFORM_WINDOW_UNVERIFIABLE/);
