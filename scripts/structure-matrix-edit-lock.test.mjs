...
assert.match(explorer, /if \(!matrixEditingEnabled \|\| !item\.id\) return;[\s\S]*setAssignmentKeys[\s\S]*toggleRoleGovernanceProcessInline/, "write handler must stop before optimistic and server updates");
assert.match(explorer, /disabled=\{!matrixEditingEnabled \|\| !item\.id \|\| pending\}/, "matrix cells must be disabled while locked");
