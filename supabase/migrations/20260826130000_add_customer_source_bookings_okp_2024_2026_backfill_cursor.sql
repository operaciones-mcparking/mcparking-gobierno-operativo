...
create or replace function public.get_customer_source_bookings_okp_2024_2026_backfill_state_m2m()
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_sync_kind constant text := 'historical_backfill_2024_2026';
...
$$;
create or replace function public.advance_customer_source_bookings_okp_2024_2026_backfill_cursor_m2m(
  p_expected_source_created_at timestamp without time zone,
  p_expected_source_id bigint,
  p_new_source_created_at timestamp without time zone,
...
declare
  ...
  v_state public.recovery_sync_state%rowtype;
  v_previous_source_created_at timestamp without time zone;
  v_previous_source_id bigint;
  v_expected_version bigint;
begin
  if p_expected_source_created_at is null or p_new_source_created_at is null then
    raise exception 'expected and new source_created_at are required' using errcode = '22023';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('customer_source_bookings_okp_historical_backfill_2024_2026_cursor', 0)
  );
  ...
  if p_new_source_created_at < p_expected_source_created_at
    or (
      p_new_source_created_at = p_expected_source_created_at
  ...
  v_previous_source_created_at := v_state.last_source_updated_at;
  ...
  where state.source_key = v_source_key
    and state.sync_kind = v_sync_kind
    and state.last_source_updated_at is not distinct from p_expected_source_created_at
  ...
  if not found then
    return pg_catalog.jsonb_build_object(
      ...
      'sourceKey', v_source_key,
      'syncKind', v_sync_kind,
      'lastSourceCreatedAt', v_previous_source_created_at,
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'sourceKey', v_source_key,
    'syncKind', v_sync_kind,
    'previousSourceCreatedAt', v_previous_source_created_at,
...
revoke all on function public.get_customer_source_bookings_okp_2024_2026_backfill_state_m2m()
  from public, anon, authenticated, service_role;

grant execute on function public.get_customer_source_bookings_okp_2024_2026_backfill_state_m2m()
  to service_role;
revoke all on function public.advance_customer_source_bookings_okp_2024_2026_backfill_cursor_m2m(
...
grant execute on function public.advance_customer_source_bookings_okp_2024_2026_backfill_cursor_m2m(
  timestamp without time zone,
  bigint,
  timestamp without time zone,
  bigint
