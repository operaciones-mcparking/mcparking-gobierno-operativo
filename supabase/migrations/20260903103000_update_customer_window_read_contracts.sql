...
as $$
  ...
  booking_summary as (
    select
      ...
      count(*) filter (where is_pack is true)::bigint as pack_count,
      count(*) filter (where is_pack is false)::bigint as non_pack_count,
      count(*) filter (
        where planned_arrival_at::date
          > pg_catalog.timezone('America/Santiago', pg_catalog.now())::date
      )::bigint as future_booking_count,
      (pg_catalog.array_agg(brand order by purchase_created_at desc nulls last, source desc, source_row_id desc))[1] as last_brand,
      (pg_catalog.array_agg(parking order by purchase_created_at desc nulls last, source desc, source_row_id desc))[1] as last_parking
    from bookings
  ),
  identity_counts as (
    select
  ...
  select case when not exists (select 1 from profile) then
    ...
    'firstPurchaseAt', (select first_purchase_at from booking_summary),
    'lastPurchaseAt', (select last_purchase_at from booking_summary),
    'purchaseCount', (select purchase_count from booking_summary),
    'totalSpend', (select total_spend from booking_summary),
    'averageTicket', (select average_ticket from booking_summary),
    'totalDurationDays', (select total_duration_days from booking_summary),
    'mcpCount', (select mcp_count from booking_summary),
    'eapCount', (select eap_count from booking_summary),
    'okpCount', (select okp_count from booking_summary),
    'packCount', (select pack_count from booking_summary),
    'nonPackCount', (select non_pack_count from booking_summary),
    'futureBookingCount', (select future_booking_count from booking_summary),
    'lastBrand', (select last_brand from booking_summary),
    'lastParking', (select last_parking from booking_summary),
...
begin
  ...
  with matches as (
    select distinct identity_link.profile_id as customer_id, p_identity_type as matched_identity_type
