-- Adds controlled mutable-row updates for WhatsApp tracking imports.
-- This migration does not modify tracking data by itself; it replaces the RPCs so future imports can update status fields.

alter table public.recovery_import_batches
  add column if not exists updated_rows integer default 0;

-- Update chunked import RPCs for n8n Seguimiento WhatsApp tracking CSV rows.
-- Existing message_id rows with changed hashes update mutable tracking status fields.
...
comment on function public.append_recovery_whatsapp_tracking_import_rows(uuid, jsonb) is
  'Appends one normalized chunk of WhatsApp tracking rows. Existing message_id rows with changed hashes update mutable tracking fields and are reported as updatedRows.';
...
begin
  ...
  return jsonb_build_object(
    ...
    'updatedRows', coalesce(v_batch.updated_rows, 0),
...
comment on function public.finish_recovery_whatsapp_tracking_import(uuid) is
  'Finishes a chunked WhatsApp tracking import batch and returns safe aggregate counters including updatedRows.';
