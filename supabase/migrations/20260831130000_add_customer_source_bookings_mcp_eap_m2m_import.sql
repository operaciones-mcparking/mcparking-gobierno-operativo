...
do $$
begin
  if to_regclass('public.customer_source_bookings_mcp_eap') is null then
    raise exception 'Required table public.customer_source_bookings_mcp_eap does not exist';
  end if;
end;
$$;
...
begin
  ...
  loop
    ...
    if v_input.source is distinct from 'MCP_EAP'
      ...
      or v_input.booking_status not in (1, 8)
