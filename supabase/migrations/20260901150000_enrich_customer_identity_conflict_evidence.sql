  ...
  loop
    ...
    select
      (v_booking.phone_normalized is not null and exists (
        select 1 from all_valid matching_phone
        where matching_phone.phone_normalized = v_booking.phone_normalized
          and matching_phone.email_normalized is not null
          and matching_phone.email_normalized is distinct from v_booking.email_normalized
      ))
      or
      (v_booking.email_normalized is not null and exists (
        select 1 from all_valid matching_email
        where matching_email.email_normalized = v_booking.email_normalized
          and matching_email.phone_normalized is not null
          and matching_email.phone_normalized is distinct from v_booking.phone_normalized
      )),
      count(distinct candidate.email_normalized) filter (
        where v_booking.phone_normalized is not null
          and candidate.phone_normalized = v_booking.phone_normalized
          and candidate.email_normalized is not null
      )::integer,
      count(distinct candidate.phone_normalized) filter (
        where v_booking.email_normalized is not null
          and candidate.email_normalized = v_booking.email_normalized
    ...
    into
      ...
      v_emails_for_phone,
      v_phones_for_email,
      v_phone_booking_count,
      v_email_booking_count
    from all_valid candidate;
