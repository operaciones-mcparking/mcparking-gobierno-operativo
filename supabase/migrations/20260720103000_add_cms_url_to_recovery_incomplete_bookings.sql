-- Add sensitive reservation URL support for recovery chat drawer actions.
-- cms_url is operational sensitive data. It must only be exposed in admin-only
-- point-in-time chat drawer actions, never in aggregate dashboards or exports.

alter table public.recovery_incomplete_bookings_import
  add column if not exists cms_url text;

comment on column public.recovery_incomplete_bookings_import.cms_url is
  'Sensitive operational reservation URL from incomplete booking source. Use only in admin recovery chat drawer actions; do not expose in aggregate views, import summaries, histories or exports.';
