-- Update incomplete bookings import RPC to persist safe intended reservation dates.
-- The app must validate the CSV and build normalized rows server-side before
-- calling this function. This RPC never stores raw CSV content, raw bform or raw PII.
