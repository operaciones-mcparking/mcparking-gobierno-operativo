-- Transactional import RPC for n8n Seguimiento WhatsApp tracking CSV rows.
-- The app must validate the CSV and build normalized rows server-side before
-- calling this function. This RPC never stores raw phones, Json_Encuesta,
-- raw CSV content, payloads or message bodies.

create or replace function public.import_recovery_whatsapp_tracking(
  p_file_name text,
  p_file_size bigint,
  p_file_hash text,
  p_summary jsonb,
...
begin
  ...
  -- Serialize tracking imports in the MVP to avoid duplicate races.
  perform pg_advisory_xact_lock(20260715162000);
  ...
  values (
    ...
    coalesce(p_summary->'missingMandatory', '[]'::jsonb),
    coalesce(p_summary->'statusCounts', '{}'::jsonb),
  ...
  set
    ...
    skipped_duplicate_rows = v_internal_duplicate_rows + v_source_duplicate_rows + v_message_duplicate_rows + v_row_hash_duplicate_rows,
...
comment on function public.import_recovery_whatsapp_tracking(text, bigint, text, jsonb, jsonb) is
  'Transactionally imports normalized n8n Seguimiento WhatsApp rows into staging tables. Validates active admin access and returns only aggregate counts.';

revoke all on function public.import_recovery_whatsapp_tracking(text, bigint, text, jsonb, jsonb) from public;
revoke execute on function public.import_recovery_whatsapp_tracking(text, bigint, text, jsonb, jsonb) from anon;
grant execute on function public.import_recovery_whatsapp_tracking(text, bigint, text, jsonb, jsonb) to authenticated;
