-- Add nullable source analytics fields to recovery purchases and mutable change events.
-- Prepared for manual review and application; this migration is not applied automatically.

begin;
alter table public.recovery_bookings_import
  add column if not exists person_count smallint,
  add column if not exists promotion_code text,
  add column if not exists promotion_discount_amount numeric(10,2),
  add column if not exists booking_paid numeric(10,2),
  add column if not exists website_source integer,
  add column if not exists sub_days_used integer;

alter table public.recovery_bookings_import
  drop constraint if exists recovery_bookings_import_analytics_fields_check;

alter table public.recovery_bookings_import
  add constraint recovery_bookings_import_analytics_fields_check
  check (
    (person_count is null or person_count >= 0)
    and (promotion_discount_amount is null or promotion_discount_amount >= 0)
    and (booking_paid is null or booking_paid >= 0)
    and (sub_days_used is null or sub_days_used >= 0)
  );

comment on column public.recovery_bookings_import.person_count is
  'Original mcp_Buchungen.Personenzahl value. Null is retained for historical rows not yet reconciled.';
comment on column public.recovery_bookings_import.promotion_code is
  'Original trimmed mcp_Buchungen.PromotionCode value; no promotion semantics are derived here.';
comment on column public.recovery_bookings_import.promotion_discount_amount is
  'Original mcp_Buchungen.PromotionCodeCalculatedValue amount.';
comment on column public.recovery_bookings_import.booking_paid is
  'Original mcp_Buchungen.BookingPaid amount, independent from booking status.';
comment on column public.recovery_bookings_import.website_source is
  'Original mcp_Buchungen.Website integer code; it is intentionally not mapped to a URL or brand.';
comment on column public.recovery_bookings_import.sub_days_used is
  'Original mcp_Buchungen.SubDaysUsed value; no payment mode is derived here.';
alter table public.recovery_import_row_changes
  add column if not exists previous_person_count smallint,
  add column if not exists current_person_count smallint,
  add column if not exists previous_promotion_code text,
  add column if not exists current_promotion_code text,
  add column if not exists previous_promotion_discount_amount numeric(10,2),
  add column if not exists current_promotion_discount_amount numeric(10,2),
  add column if not exists previous_booking_paid numeric(10,2),
  add column if not exists current_booking_paid numeric(10,2),
  add column if not exists previous_website_source integer,
  add column if not exists current_website_source integer,
  add column if not exists previous_sub_days_used integer,
  add column if not exists current_sub_days_used integer;

do $$
begin
  if to_regprocedure('public.import_recovery_purchases_m2m(text,bigint,text,jsonb,jsonb)') is null then
    raise exception 'Required function public.import_recovery_purchases_m2m(text,bigint,text,jsonb,jsonb) does not exist';
  end if;

  if to_regprocedure('public.import_recovery_purchases(text,bigint,text,jsonb,jsonb)') is null then
    raise exception 'Required function public.import_recovery_purchases(text,bigint,text,jsonb,jsonb) does not exist';
  end if;
  ...
  if not exists (
    ...
    where table_schema = 'public'
      and table_name = 'recovery_import_batches'
      and column_name = 'source_duplicate_rows'
  ) or not exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'recovery_import_batches'
      and column_name = 'booking_duplicate_rows'
  ) then
    raise exception 'Required duplicate count columns do not exist on public.recovery_import_batches';
  end if;
end;
$$;
create or replace function public.import_recovery_purchases_m2m(
  p_file_name text,
  p_file_size bigint,
  p_file_hash text,
  p_summary jsonb,
...
begin
  ...
  if v_existing_batch_id is not null then
    return jsonb_build_object(
      ...
      'sourceDuplicateRows', v_rows_received,
  ...
  with input_rows as (
    select
      ...
      price,
      person_count,
      nullif(trim(promotion_code), '') as promotion_code,
  ...
  checked_rows as (
    select
      ...
      existing_source.person_count as existing_source_person_count,
      existing_source.promotion_code as existing_source_promotion_code,
      existing_source.promotion_discount_amount as existing_source_promotion_discount_amount,
      existing_source.booking_paid as existing_source_booking_paid,
      existing_source.website_source as existing_source_website_source,
      existing_source.sub_days_used as existing_source_sub_days_used,
    ...
    left join lateral (
      select
        ...
        existing_source_row.person_count,
        existing_source_row.promotion_code,
        existing_source_row.promotion_discount_amount,
        existing_source_row.booking_paid,
        existing_source_row.website_source,
        existing_source_row.sub_days_used
  ...
  classified_rows as (
    select
      ...
      (
        ...
        or existing_source_person_count is distinct from person_count
        or existing_source_promotion_code is distinct from promotion_code
        or existing_source_promotion_discount_amount is distinct from promotion_discount_amount
        or existing_source_booking_paid is distinct from booking_paid
        or existing_source_website_source is distinct from website_source
        or existing_source_sub_days_used is distinct from sub_days_used
  ...
  updated_rows as (
    ...
    set
      ...
      person_count = updateable_rows.person_count,
      promotion_code = updateable_rows.promotion_code,
      promotion_discount_amount = updateable_rows.promotion_discount_amount,
      booking_paid = updateable_rows.booking_paid,
      website_source = updateable_rows.website_source,
      sub_days_used = updateable_rows.sub_days_used,
    ...
    returning
      ...
      updateable_rows.existing_source_person_count as previous_person_count,
      target.person_count as current_person_count,
      updateable_rows.existing_source_promotion_code as previous_promotion_code,
      target.promotion_code as current_promotion_code,
      updateable_rows.existing_source_promotion_discount_amount as previous_promotion_discount_amount,
      target.promotion_discount_amount as current_promotion_discount_amount,
      updateable_rows.existing_source_booking_paid as previous_booking_paid,
      target.booking_paid as current_booking_paid,
      updateable_rows.existing_source_website_source as previous_website_source,
      target.website_source as current_website_source,
      updateable_rows.existing_source_sub_days_used as previous_sub_days_used,
      target.sub_days_used as current_sub_days_used
  ),
  insertable_rows as (
    select *
    from classified_rows
  ...
  inserted_change_events as (
    insert into public.recovery_import_row_changes (
      ...
      current_person_count, current_promotion_code, current_promotion_discount_amount,
      current_booking_paid, current_website_source, current_sub_days_used
    )
    select
      ...
      array['customer_id','location_code','arrival_date','departure_date','duration_days','booking_created_at','booking_number','booking_status','paying_status','is_valid_purchase','price','parking_code','identity_email_hash','identity_phone_hash','person_count','promotion_code','promotion_discount_amount','booking_paid','website_source','sub_days_used','row_hash']::text[],
      ...
      inserted_rows.person_count, inserted_rows.promotion_code, inserted_rows.promotion_discount_amount,
      inserted_rows.booking_paid, inserted_rows.website_source, inserted_rows.sub_days_used
    from inserted_rows
  ...
  updated_change_events as (
    insert into public.recovery_import_row_changes (
      ...
      previous_person_count, current_person_count, previous_promotion_code, current_promotion_code,
      previous_promotion_discount_amount, current_promotion_discount_amount,
      previous_booking_paid, current_booking_paid, previous_website_source, current_website_source,
      previous_sub_days_used, current_sub_days_used
    )
    select
      ...
      array_remove(array[
        ...
        case when updated_rows.previous_person_count is distinct from updated_rows.current_person_count then 'person_count' end,
        case when updated_rows.previous_promotion_code is distinct from updated_rows.current_promotion_code then 'promotion_code' end,
        case when updated_rows.previous_promotion_discount_amount is distinct from updated_rows.current_promotion_discount_amount then 'promotion_discount_amount' end,
        case when updated_rows.previous_booking_paid is distinct from updated_rows.current_booking_paid then 'booking_paid' end,
        case when updated_rows.previous_website_source is distinct from updated_rows.current_website_source then 'website_source' end,
        case when updated_rows.previous_sub_days_used is distinct from updated_rows.current_sub_days_used then 'sub_days_used' end,
      ...
      updated_rows.previous_person_count, updated_rows.current_person_count,
      updated_rows.previous_promotion_code, updated_rows.current_promotion_code,
      updated_rows.previous_promotion_discount_amount, updated_rows.current_promotion_discount_amount,
      updated_rows.previous_booking_paid, updated_rows.current_booking_paid,
      updated_rows.previous_website_source, updated_rows.current_website_source,
      updated_rows.previous_sub_days_used, updated_rows.current_sub_days_used
    from updated_rows
    on conflict (batch_id, source, entity_id, operation) do nothing
    returning id
  ),
  ...
  return jsonb_build_object(
    ...
    'internalDuplicateRows', v_internal_duplicate_rows,
...
$$;
comment on function public.import_recovery_purchases_m2m(text, bigint, text, jsonb, jsonb) is
  'Internal service-role-only implementation for normalized recovery purchase batches from trusted server-side sources.';

revoke all on function public.import_recovery_purchases_m2m(text, bigint, text, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.import_recovery_purchases_m2m(text, bigint, text, jsonb, jsonb) to service_role;
...
begin
  ...
  return public.import_recovery_purchases_m2m(
    p_file_name,
    p_file_size,
    p_file_hash,
    p_summary,
...
comment on function public.import_recovery_purchases(text, bigint, text, jsonb, jsonb) is
  'Admin-only entry point for normalized recovery purchase imports. Delegates to the shared transactional implementation.';

revoke all on function public.import_recovery_purchases(text, bigint, text, jsonb, jsonb) from public;
revoke execute on function public.import_recovery_purchases(text, bigint, text, jsonb, jsonb) from anon, service_role;
