...
begin
  ...
  with period_bookings as materialized (
    select link.profile_id as customer_id
...
comment on function public.customer_window_get_purchase_period_facets(
  date, date, text, text, text, text
) is 'Returns non-PII Customer Window facets for unique customers with confirmed purchases in an inclusive Santiago calendar-date period.';
