  ...
  skipped_checked_rows as (
    ...
    where not exists (
      ...
      and not exists (
        select 1
        from insertable_rows
        where insertable_rows.input_position = checked_rows.input_position
      )
      and not exists (
        select 1
        from conflict_input_rows
        where conflict_input_rows.input_position = checked_rows.input_position
      )
  ),
  classified_duplicate_rows as (
    select
      skipped_checked_rows.*,
      case
        when existing_source_id is not null and existing_source_row_hash is not distinct from row_hash then 'source'
        when existing_message_id is not null then 'message'
        when existing_hash_id is not null then 'row_hash'
        else 'message'
      end as duplicate_kind
    from skipped_checked_rows
  ),
  stats as (
    select
      ...
      (
        select count(*)
        from classified_duplicate_rows
        where duplicate_kind = 'source'
      )::integer as source_duplicate_rows,
      (
        select count(*)
        from classified_duplicate_rows
        where duplicate_kind = 'message'
      )::integer as message_duplicate_rows,
      (
        select count(*)
        from classified_duplicate_rows
        where duplicate_kind = 'row_hash'
      )::integer as row_hash_duplicate_rows,
      (select count(*) from conflict_input_rows)::integer as conflict_rows,
...
comment on function public.append_recovery_whatsapp_tracking_import_rows(uuid, jsonb) is
  'Appends one normalized chunk of WhatsApp tracking rows. Existing message_id rows with changed hashes update mutable tracking fields only when the incoming row is not stale or regressive. Duplicate counters are mutually exclusive so skippedDuplicateRows is not double-counted.';
