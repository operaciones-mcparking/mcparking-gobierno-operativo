...
begin
  ...
  with scoped_members as materialized (
    ...
    select
      ...
      coalesce(
        (pg_catalog.array_agg(observation.raw_value
          order by observation.source_created_at desc nulls last, observation.source,
            observation.source_row_id desc)
      ...
      (pg_catalog.array_agg(observation.source
        order by observation.source_created_at desc nulls last, observation.source,
          observation.source_row_id desc))[1] as source,
      (pg_catalog.array_agg(observation.source_row_id
        order by observation.source_created_at desc nulls last, observation.source,
          observation.source_row_id desc))[1] as source_row_id,
      (pg_catalog.array_agg(observation.source_booking_code
        order by observation.source_created_at desc nulls last, observation.source,
          observation.source_row_id desc))[1] as booking_code,
      max(observation.source_created_at) as observed_at,
      min(observation.source_created_at) as first_seen_at,
      max(observation.source_created_at) as last_seen_at,
      count(distinct observation.source)::bigint as source_count,
      count(distinct (observation.source, observation.source_row_id))::bigint as booking_count,
    ...
    cross join lateral (
      select
        'OKP'::text as source,
        source_booking.source_row_id,
        source_booking.source_booking_code,
        source_booking.email_raw,
        source_booking.email_normalized,
        source_booking.source_created_at
      from public.customer_source_bookings_okp source_booking
      where source_booking.phone_normalized = pivot.normalized_value
        and (
      ...
      where source_booking.phone_normalized = pivot.normalized_value
        and source_booking.source = 'MCP_EAP'
    ...
    cross join lateral (
      select
        ...
        source_booking.source_row_id,
        source_booking.source_booking_code,
        source_booking.phone_raw,
        source_booking.phone_normalized,
        source_booking.source_created_at
      from public.customer_source_bookings_okp source_booking
      where source_booking.email_normalized = pivot.normalized_value
        and (
          (source_booking.status_raw = 'PAGADA'
            and source_booking.is_confirmed is true
            and source_booking.is_paid is true)
    ...
    select
      ...
      null::timestamp without time zone as observed_at,
  ...
  select pg_catalog.jsonb_build_object(
    'emails', coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
      ...
      'type', contact.identity_type,
      'source', contact.source,
      'sourceRowId', contact.source_row_id,
      'bookingCode', contact.booking_code,
...
comment on function public.customer_window_v2_get_identity_resolution_detail(text) is
  'Read-only active-snapshot identity-resolution evidence and one-hop cross-source group-pivot observed/historical contacts for one related-review group.';
