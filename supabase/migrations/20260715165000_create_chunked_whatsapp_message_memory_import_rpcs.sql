-- Chunked import RPCs for n8n WhatsApp Message Memory CSV rows.
-- These functions keep one logical batch per file while allowing the app to
-- append normalized rows in smaller chunks. They never store raw Message,
-- text_summary, raw phones, raw CSV content, payloads or message bodies.

create or replace function public.start_recovery_whatsapp_message_memory_import(
  p_file_name text,
  p_file_size bigint,
  p_file_hash text,
  p_summary jsonb
...
begin
  ...
  where rib.file_hash = v_file_hash
    and rib.import_type = 'whatsapp_message_memory_csv'
...
comment on function public.start_recovery_whatsapp_message_memory_import(text, bigint, text, jsonb) is
  'Starts a chunked WhatsApp Message Memory import batch. Returns an existing imported batch for duplicate file hashes.';

create or replace function public.append_recovery_whatsapp_message_memory_import_rows(
  p_batch_id uuid,
  p_rows jsonb
)
returns jsonb
...
begin
  ...
  where id = p_batch_id
    and import_type = 'whatsapp_message_memory_csv'
  for update;

  if not found then
    raise exception 'message memory import batch not found' using errcode = '22023';
  end if;

  if v_batch.status <> 'importing' then
    raise exception 'message memory import batch is not importing' using errcode = '22023';
  end if;
  ...
  valid_input as (
    ...
    where conversation_id is not null
      ...
      and (processing_time_seconds is null or processing_time_seconds >= 0)
  ),
  row_hash_ranked as (
    select
      valid_input.*,
  ...
  checked_rows as (
    ...
    left join lateral (
      ...
      from public.recovery_whatsapp_message_memory_import existing_hash_row
  ...
  inserted_rows as (
    insert into public.recovery_whatsapp_message_memory_import (
      batch_id,
      conversation_id,
...
comment on function public.append_recovery_whatsapp_message_memory_import_rows(uuid, jsonb) is
  'Appends one normalized chunk of WhatsApp Message Memory rows to an importing recovery batch and returns safe aggregate counts for that chunk.';

create or replace function public.finish_recovery_whatsapp_message_memory_import(
  p_batch_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if not found then
    raise exception 'message memory import batch not found or not importing' using errcode = '22023';
  end if;
...
comment on function public.finish_recovery_whatsapp_message_memory_import(uuid) is
  'Finishes a chunked WhatsApp Message Memory import batch and returns safe aggregate counters.';

create or replace function public.fail_recovery_whatsapp_message_memory_import(
  p_batch_id uuid,
...
comment on function public.fail_recovery_whatsapp_message_memory_import(uuid, text) is
  'Marks a chunked WhatsApp Message Memory import batch as failed with a truncated safe error message.';

revoke all on function public.start_recovery_whatsapp_message_memory_import(text, bigint, text, jsonb) from public;
revoke execute on function public.start_recovery_whatsapp_message_memory_import(text, bigint, text, jsonb) from anon;
grant execute on function public.start_recovery_whatsapp_message_memory_import(text, bigint, text, jsonb) to authenticated;

revoke all on function public.append_recovery_whatsapp_message_memory_import_rows(uuid, jsonb) from public;
revoke execute on function public.append_recovery_whatsapp_message_memory_import_rows(uuid, jsonb) from anon;
grant execute on function public.append_recovery_whatsapp_message_memory_import_rows(uuid, jsonb) to authenticated;

revoke all on function public.finish_recovery_whatsapp_message_memory_import(uuid) from public;
revoke execute on function public.finish_recovery_whatsapp_message_memory_import(uuid) from anon;
grant execute on function public.finish_recovery_whatsapp_message_memory_import(uuid) to authenticated;

revoke all on function public.fail_recovery_whatsapp_message_memory_import(uuid, text) from public;
revoke execute on function public.fail_recovery_whatsapp_message_memory_import(uuid, text) from anon;
grant execute on function public.fail_recovery_whatsapp_message_memory_import(uuid, text) to authenticated;
