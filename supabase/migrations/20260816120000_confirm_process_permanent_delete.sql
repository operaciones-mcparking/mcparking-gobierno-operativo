-- Require a fixed explicit confirmation before permanently deleting one process.
...
declare
  v_process_name text;
begin
  if p_process_id is null or p_confirmation_name is distinct from 'CONFIRMAR' then
    raise exception 'Process id and exact confirmation text are required';
  end if;
