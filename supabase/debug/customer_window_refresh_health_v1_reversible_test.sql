-- Reversible PostgreSQL certification for Customer Window refresh health V1.
-- Run only against a non-production database with the Related Review base schema installed.
begin;
...
-- Catalog, ACL, CAS, status, and retention contracts.
do $test$
declare
  v_run_1 uuid := '11111111-1111-4111-8111-111111111111'::uuid;
  v_run_2 uuid := '22222222-2222-4222-8222-222222222222'::uuid;
  v_run_3 uuid := '33333333-3333-4333-8333-333333333333'::uuid;
  v_run_4 uuid := '44444444-4444-4444-8444-444444444444'::uuid;
  v_result jsonb;
  v_before_superseded integer;
  v_after_superseded integer;
...
begin
  if not exists (
    ...
    where namespace.nspname = 'public'
      and relation.relname = 'customer_related_review_refresh_state'
      and relation.relrowsecurity
  ) then
    raise exception 'refresh health RLS contract failed';
  end if;

  if exists (
    select 1
    from (values ('service_role'), ('customer_related_review_builder')) role_name(name)
    cross join (values ('SELECT'), ('INSERT'), ('UPDATE'), ('DELETE'), ('TRUNCATE'),
      ('REFERENCES'), ('TRIGGER')) privilege_name(name)
    where pg_catalog.has_table_privilege(role_name.name,
      'public.customer_related_review_refresh_state', privilege_name.name)
  ) then
    raise exception 'refresh health direct table ACL contract failed';
  end if;

  if not pg_catalog.has_function_privilege('customer_related_review_builder',
      'public.customer_related_review_refresh_start_v1_m2m(uuid)', 'EXECUTE')
    or not pg_catalog.has_function_privilege('customer_related_review_builder',
      'public.customer_related_review_refresh_heartbeat_v1_m2m(uuid)', 'EXECUTE')
    or not pg_catalog.has_function_privilege('customer_related_review_builder',
      'public.customer_related_review_refresh_finish_v1_m2m(uuid,boolean,text,text,boolean,integer,uuid,text)', 'EXECUTE')
    or pg_catalog.has_function_privilege('service_role',
      'public.customer_related_review_refresh_start_v1_m2m(uuid)', 'EXECUTE')
    or not pg_catalog.has_function_privilege('service_role',
      'public.customer_window_get_refresh_health_v1_m2m()', 'EXECUTE')
    or pg_catalog.has_function_privilege('customer_related_review_builder',
      'public.customer_window_get_refresh_health_v1_m2m()', 'EXECUTE') then
    raise exception 'refresh health function ACL contract failed';
  end if;
  ...
  if exists (
    ...
    where procedure.oid in (
      to_regprocedure('public.customer_related_review_refresh_start_v1_m2m(uuid)'),
      to_regprocedure('public.customer_related_review_refresh_heartbeat_v1_m2m(uuid)'),
      to_regprocedure('public.customer_related_review_refresh_finish_v1_m2m(uuid,boolean,text,text,boolean,integer,uuid,text)'),
      to_regprocedure('public.customer_window_get_refresh_health_v1_m2m()')
    )
      and acl.grantee = 0
      and acl.privilege_type = 'EXECUTE'
  ) then
    raise exception 'refresh health PUBLIC execute contract failed';
  end if;
  ...
  if v_active_count <> 1 then
    raise exception 'fixture requires exactly one active Related Review snapshot';
  end if;

  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'status' <> 'error'
    or v_result->>'lastErrorCode' <> 'telemetry_unavailable'
    or (v_result->>'containsPii')::boolean then
    raise exception 'missing telemetry fail-closed contract failed';
  end if;

  v_result := public.customer_related_review_refresh_start_v1_m2m(v_run_1);
  if not (v_result->>'ok')::boolean or v_result->>'runId' <> v_run_1::text then
    raise exception 'start contract failed';
  end if;
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'status' <> 'refreshing' then
    raise exception 'refreshing status contract failed';
  end if;

  update public.customer_related_review_refresh_state
  set heartbeat_at = pg_catalog.now() - interval '121 seconds'
  where rule_key = 'RELATED_REVIEW_MCP_EAP_V1';
  v_result := public.customer_related_review_refresh_start_v1_m2m(v_run_2);
  if not (v_result->>'ok')::boolean then raise exception 'stale takeover failed'; end if;
  v_result := public.customer_related_review_refresh_heartbeat_v1_m2m(v_run_1);
  if (v_result->>'ok')::boolean or v_result->>'code' <> 'refresh_run_not_current' then
    raise exception 'old heartbeat CAS failed';
  end if;
  v_result := public.customer_related_review_refresh_finish_v1_m2m(
    v_run_1, true, null, null, false, null, null, null);
  if (v_result->>'ok')::boolean then raise exception 'old finish CAS failed'; end if;

  v_result := public.customer_related_review_refresh_heartbeat_v1_m2m(v_run_2);
  if not (v_result->>'ok')::boolean then raise exception 'current heartbeat failed'; end if;
  v_result := public.customer_related_review_refresh_finish_v1_m2m(
    v_run_2, true, null, null, true, 1,
    '55555555-5555-4555-8555-555555555555'::uuid, null);
  if not (v_result->>'ok')::boolean or v_result->>'status' <> 'success' then
    raise exception 'success finish failed';
  end if;
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'status' <> 'healthy'
    or v_result->>'retentionStatus' not in ('active', 'draining') then
    raise exception 'healthy status contract failed';
  end if;

  update public.customer_related_review_refresh_state
  set last_success_at = pg_catalog.now() - interval '46 minutes';
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'status' <> 'stale' then raise exception 'stale status contract failed'; end if;

  v_result := public.customer_related_review_refresh_start_v1_m2m(v_run_3);
  update public.customer_related_review_refresh_state
  set heartbeat_at = pg_catalog.now() - interval '121 seconds'
  where run_id = v_run_3;
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'status' <> 'error'
    or v_result->>'lastErrorCode' <> 'refresh_heartbeat_expired' then
    raise exception 'expired heartbeat contract failed';
  end if;
  v_result := public.customer_related_review_refresh_finish_v1_m2m(
    v_run_3, false, 'ready_audit_failed', 'ready-audit', false, null, null, null);
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'status' <> 'error'
    or v_result->>'lastErrorCode' <> 'ready_audit_failed' then
    raise exception 'error finish contract failed';
  end if;

  v_result := public.customer_related_review_refresh_start_v1_m2m(v_run_4);
  v_result := public.customer_related_review_refresh_finish_v1_m2m(
    v_run_4, false, 'retention_failed', 'retention', true, 0, null, '55P03');
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_result->>'retentionStatus' <> 'error' then
    raise exception 'retention error contract failed';
  end if;

  v_before_superseded := (select count(*)::integer
    from public.customer_related_review_snapshots snapshot
    where snapshot.rule_key = 'RELATED_REVIEW_MCP_EAP_V1'
  ...
  select pg_catalog.gen_random_uuid(), 'RELATED_REVIEW_MCP_EAP_V1', 'health-harness',
    'superseded', pg_catalog.now() - interval '3 hours',
    pg_catalog.now() - interval '3 hours', pg_catalog.now() - interval '2 hours',
    pg_catalog.now() - interval '1 hour', repeat('0', 64), 0, 0, 0, 0, 0, 0
  from generate_series(1, 6);
  v_after_superseded := (select count(*)::integer
    from public.customer_related_review_snapshots snapshot
    where snapshot.rule_key = 'RELATED_REVIEW_MCP_EAP_V1'
      and snapshot.status = 'superseded');
  v_result := public.customer_window_get_refresh_health_v1_m2m();
  if v_after_superseded - v_before_superseded <> 6
    or (v_result->>'retentionRemaining')::integer
      <> greatest(v_after_superseded - 5, 0) then
    raise exception 'live retention backlog contract failed';
  end if;
  ...
  begin
    ...
    v_result := public.customer_window_get_refresh_health_v1_m2m();
    if v_result->>'status' <> 'error'
      or v_result->>'lastErrorCode' <> 'active_snapshot_count_invalid' then
      raise exception 'invalid active count contract failed';
    end if;
    raise exception 'rollback_invalid_active_fixture';
  exception when raise_exception then
    if sqlerrm <> 'rollback_invalid_active_fixture' then raise; end if;
  end;
end;
$test$;

select public.customer_window_get_refresh_health_v1_m2m() as refresh_health_contract;

rollback;

do $postcheck$
begin
  if to_regclass('public.customer_related_review_refresh_state') is not null
    or to_regprocedure('public.customer_related_review_refresh_start_v1_m2m(uuid)') is not null
    or to_regprocedure('public.customer_related_review_refresh_heartbeat_v1_m2m(uuid)') is not null
    or to_regprocedure('public.customer_related_review_refresh_finish_v1_m2m(uuid,boolean,text,text,boolean,integer,uuid,text)') is not null
    or to_regprocedure('public.customer_window_get_refresh_health_v1_m2m()') is not null then
    raise exception 'refresh health reversible cleanup failed';
  end if;
end;
$postcheck$;

select
  to_regclass('public.customer_related_review_refresh_state') is null as table_absent,
  to_regprocedure('public.customer_related_review_refresh_start_v1_m2m(uuid)') is null
    as start_rpc_absent,
  to_regprocedure('public.customer_related_review_refresh_heartbeat_v1_m2m(uuid)') is null
    as heartbeat_rpc_absent,
  to_regprocedure('public.customer_related_review_refresh_finish_v1_m2m(uuid,boolean,text,text,boolean,integer,uuid,text)') is null
    as finish_rpc_absent,
  to_regprocedure('public.customer_window_get_refresh_health_v1_m2m()') is null
    as read_rpc_absent,
