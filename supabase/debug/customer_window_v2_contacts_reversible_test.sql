...
begin
  foreach v_signature in array array[
    'public.customer_window_v2_list_representations_by_purchase_period(date,date,integer,integer)',
    ...
    if v_oid is null then
      raise exception 'Missing Customer Window v2 contact RPC: %', v_signature;
    end if;
    ...
    v_expected_defaults := case
      when v_signature like '%get_representation_summary%' then 0
      else 2
    end;
    ...
    if v_proc.prorettype <> 'jsonb'::pg_catalog.regtype
      ...
      or v_proc.pronargdefaults <> v_expected_defaults
      or v_proc.proconfig is null
      or not exists (
        select 1
        from pg_catalog.unnest(v_proc.proconfig) setting
      ...
      raise exception 'Customer Window v2 contact RPC catalog contract failed: %', v_signature;
    end if;
  end loop;
end;
$$;
...
select
  pg_catalog.to_regprocedure(
    'public.customer_window_v2_list_representations_by_purchase_period(date,date,integer,integer)'
  ) is not null as list_rpc_exists,
  pg_catalog.to_regprocedure(
    'public.customer_window_v2_get_representation_summary(text,text)'
...
declare
  v_confirmed_id text;
  v_related_single_id text;
  v_related_multi_id text;
  v_latest_day date;
  v_confirmed_summary jsonb;
  v_related_single_summary jsonb;
  v_related_multi_summary jsonb;
...
begin
  ...
  from public.customer_window_mcp_eap_representations_v2 representation
  join public.customer_identity_links identity
    on identity.profile_id = representation.customer_id
   and identity.status = 'active'
   and identity.identity_type in ('email', 'phone')
  ...
  with related_counts as (
    select
      representation.representation_id,
      count(distinct booking.email_normalized)
        filter (where nullif(booking.email_normalized, '') is not null) as email_count,
      count(distinct booking.phone_normalized)
        filter (where nullif(booking.phone_normalized, '') is not null) as phone_count
    from public.customer_window_mcp_eap_representations_v2 representation
    join public.customer_source_bookings_mcp_eap booking
      on booking.source = representation.source
     and booking.source_row_id = representation.source_row_id
  ...
  select related.representation_id
  into v_related_single_id
  from related_counts related
  where related.email_count = 1
    and related.phone_count = 1
  order by related.representation_id
  limit 1;
  ...
  select related.representation_id
  into v_related_multi_id
  from related_counts related
  where related.email_count > 1 or related.phone_count > 1
  order by related.email_count + related.phone_count, related.representation_id
  limit 1;

  if v_confirmed_id is null or v_related_single_id is null then
    raise exception 'Dynamic confirmed-contact and related-single samples are required';
  end if;

  v_confirmed_summary := public.customer_window_v2_get_representation_summary(
    'confirmed_customer', v_confirmed_id
  );
  v_related_single_summary := public.customer_window_v2_get_representation_summary(
    'related_review', v_related_single_id
  );
  ...
  v_related_bookings := public.customer_window_v2_list_representation_bookings(
    'related_review', v_related_single_id, 1, 100
  );

  if v_confirmed_summary -> 'contactSummary' ->> 'semantics' <> 'direct'
    or pg_catalog.jsonb_array_length(v_confirmed_summary -> 'directEmails')
      <> (v_confirmed_summary -> 'contactSummary' ->> 'emailCount')::integer
    or pg_catalog.jsonb_array_length(v_confirmed_summary -> 'directPhones')
      <> (v_confirmed_summary -> 'contactSummary' ->> 'phoneCount')::integer
    or pg_catalog.jsonb_array_length(v_confirmed_summary -> 'observedEmails') <> 0
    or pg_catalog.jsonb_array_length(v_confirmed_summary -> 'observedPhones') <> 0
    or v_related_single_summary -> 'contactSummary' ->> 'semantics' <> 'observed'
    or pg_catalog.jsonb_array_length(v_related_single_summary -> 'directEmails') <> 0
    or pg_catalog.jsonb_array_length(v_related_single_summary -> 'directPhones') <> 0
    or pg_catalog.jsonb_array_length(v_related_single_summary -> 'observedEmails')
      <> (v_related_single_summary -> 'contactSummary' ->> 'emailCount')::integer
    or pg_catalog.jsonb_array_length(v_related_single_summary -> 'observedPhones')
      <> (v_related_single_summary -> 'contactSummary' ->> 'phoneCount')::integer
    or (
      (v_related_single_summary -> 'contactSummary' ->> 'emailCount')::integer = 1
      and v_related_single_summary -> 'contactSummary' ->> 'singleEmail'
        is distinct from v_related_single_summary -> 'observedEmails' -> 0 ->> 'value'
    )
    or (
      (v_related_single_summary -> 'contactSummary' ->> 'phoneCount')::integer = 1
      and v_related_single_summary -> 'contactSummary' ->> 'singlePhone'
        is distinct from v_related_single_summary -> 'observedPhones' -> 0 ->> 'value'
    ) then
    raise exception 'Customer Window v2 contact summary runtime contract failed';
  end if;

  if v_related_multi_id is not null then
    v_related_multi_summary := public.customer_window_v2_get_representation_summary(
      'related_review', v_related_multi_id
    );
    if (
      (v_related_multi_summary -> 'contactSummary' ->> 'emailCount')::integer > 1
      and v_related_multi_summary -> 'contactSummary' ->> 'singleEmail' is not null
    ) or (
      (v_related_multi_summary -> 'contactSummary' ->> 'phoneCount')::integer > 1
      and v_related_multi_summary -> 'contactSummary' ->> 'singlePhone' is not null
    ) then
      raise exception 'Customer Window v2 multi-contact summary invented a primary contact';
    end if;
  end if;
  ...
  with returned as (
    ...
    from pg_catalog.jsonb_array_elements(
      (v_confirmed_bookings -> 'items') || (v_related_bookings -> 'items')
    ) item(value)
  )
  select count(*)::bigint
  into v_mismatch_count
  from returned
  left join public.customer_source_bookings_mcp_eap booking
    on booking.source = returned.value ->> 'source'
   and booking.source_row_id = (returned.value ->> 'sourceRowId')::bigint
  where booking.source_row_id is null
    or returned.value ->> 'email' is distinct from coalesce(
      nullif(pg_catalog.btrim(booking.email_raw), ''),
      nullif(booking.email_normalized, '')
    )
    or returned.value ->> 'phone' is distinct from coalesce(
      nullif(pg_catalog.btrim(booking.phone_raw), ''),
      nullif(booking.phone_normalized, '')
    );

  if v_mismatch_count <> 0 then
    raise exception 'Customer Window v2 per-booking contact parity failed';
  end if;

  select max(booking.source_created_at)::date
  into v_latest_day
  from public.customer_source_bookings_mcp_eap booking
  join public.customer_window_mcp_eap_representations_v2 representation
  ...
  if v_latest_day is null then
    raise exception 'A dynamic purchase-period sample is required';
  end if;

  v_list := public.customer_window_v2_list_representations_by_purchase_period(
    v_latest_day, v_latest_day, 1, 25
  );

  if exists (
    select 1
    from pg_catalog.jsonb_array_elements(v_list -> 'items') item(value)
    where item.value -> 'contactSummary' is null
      or item.value -> 'contactSummary' ->> 'semantics'
        is distinct from case item.value ->> 'representationType'
          when 'confirmed_customer' then 'direct'
          else 'observed'
        end
      or (
        (item.value -> 'contactSummary' ->> 'emailCount')::integer <> 1
        and item.value -> 'contactSummary' ->> 'singleEmail' is not null
      )
      or (
        (item.value -> 'contactSummary' ->> 'phoneCount')::integer <> 1
        and item.value -> 'contactSummary' ->> 'singlePhone' is not null
      )
  ) then
    raise exception 'Customer Window v2 list contact summary contract failed';
  end if;
end;
$$;

select
  true as catalog_contract_ok,
  true as runtime_contact_contract_ok,
  true as per_booking_contact_parity_ok;
...
select
  ...
  and pg_catalog.strpos(
    ...
    '''email'', paged.email'
  ) = 0 as contacts_removed_after_rollback;
